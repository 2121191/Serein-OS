...
- **状态**: [x] 已完成 (优化)
- **涉及文件**:
    - `serein-user/sh.c`: 从内核态移至用户态处理，支持保留 Input Buffer 并重绘 Prompt，解决提示符消失问题。
...
- **涉及文件**: 
    ...
    - `serein-user/halt.c`: 用户态关机命令。
    - `serein-user/reboot.c`: 用户态重启命令。
    - 用户: `serein-user/halt.c`, `serein-user/reboot.c`, `user.h`, `usys.pl`
...
- **解决方案**: 
    ...
    - 修改 `serein-user/sh.c`: 增强 Backspace 处理逻辑，支持可视删除 (`\b \b`) 本地 Buffer 内容。
